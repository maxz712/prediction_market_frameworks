...
class PolymarketConfig(BaseModel):
    ...
    wallet_proxy_address: str | None = Field(
        None, description="Wallet proxy address for funder operations (optional)"
    )
    ...
    max_total_results: int = Field(
        default=10000, description="Maximum total results to prevent memory issues"
    )

    # Feature flags
    enable_auto_pagination: bool = Field(
        default=False, description="Automatically paginate through all results"
    )
    enable_response_caching: bool = Field(
        default=False, description="Enable response caching"
    )
    warn_large_requests: bool = Field(
        default=True, description="Warn when requesting large datasets"
    )
    ...
    log_memory_usage: bool = Field(
        default=False, description="Log memory usage statistics"
    )
    performance_log_threshold_ms: float = Field(
        default=1000.0, description="Log operations slower than this threshold"
    )
    log_level: str = Field(
        default="INFO", description="Logging level for performance metrics"
    )
    log_format: str = Field(
        default="structured", description="Log format: 'structured' or 'simple'"
    )
    ...
    enable_rate_limiting: bool = Field(
        default=True, description="Enable client-side rate limiting"
    )
    rate_limiter_type: str = Field(
        default="token_bucket",
        description="Rate limiter type: 'token_bucket' or 'sliding_window'",
    )
    requests_per_second: float = Field(
        default=5.0, description="Max requests per second for token bucket rate limiter"
    )
    burst_capacity: int | None = Field(
        default=None,
        description="Burst capacity for token bucket (defaults to 2x rate)",
    )
    requests_per_window: int = Field(
        default=100,
        description="Max requests per window for sliding window rate limiter",
    )
    window_size_seconds: int = Field(
        default=60, description="Window size in seconds for sliding window rate limiter"
    )
    rate_limit_per_host: bool = Field(
        default=True, description="Apply rate limiting per host (vs globally)"
    )
    rate_limit_timeout: float | None = Field(
        default=30.0,
        description="Max time to wait for rate limit (None for no timeout)",
    )

    # SDK metadata
    sdk_version: str = Field(
        default="0.1.0", description="SDK version for User-Agent header"
    )

    # Enhanced authentication settings
    enable_signature_validation: bool = Field(
        default=True, description="Enable request signature validation"
    )
    signature_method: str = Field(
        default="hmac", description="Default signature method: 'hmac' or 'eip712'"
    )
    nonce_tracking_enabled: bool = Field(
        default=True, description="Enable nonce tracking to prevent replay attacks"
    )
    max_signature_age_seconds: int = Field(
        default=300, description="Maximum age for request signatures in seconds"
    )
    ...
    def from_env(
        ...
        rate_limit_timeout_env: str = "POLYMARKET_RATE_LIMIT_TIMEOUT",
    ) -> "PolymarketConfig":
        ...
        if enable_auto_pagination_str:
            config_data["enable_auto_pagination"] = (
                enable_auto_pagination_str.lower() in ("true", "1", "yes")
            )

        enable_response_caching_str = os.getenv(enable_response_caching_env)
        if enable_response_caching_str:
            config_data["enable_response_caching"] = (
                enable_response_caching_str.lower() in ("true", "1", "yes")
            )

        warn_large_requests_str = os.getenv(warn_large_requests_env)
        if warn_large_requests_str:
            config_data["warn_large_requests"] = warn_large_requests_str.lower() in (
                "true",
                "1",
                "yes",
            )
        ...
        if enable_performance_logging_str:
            config_data["enable_performance_logging"] = (
                enable_performance_logging_str.lower() in ("true", "1", "yes")
            )

        log_memory_usage_str = os.getenv(log_memory_usage_env)
        if log_memory_usage_str:
            config_data["log_memory_usage"] = log_memory_usage_str.lower() in (
                "true",
                "1",
                "yes",
            )
        ...
        if performance_log_threshold_ms_str:
            config_data["performance_log_threshold_ms"] = float(
                performance_log_threshold_ms_str
            )
        ...
        if enable_console_logging_str:
            config_data["enable_console_logging"] = (
                enable_console_logging_str.lower() in ("true", "1", "yes")
            )
        ...
        if enable_rate_limiting_str:
            config_data["enable_rate_limiting"] = enable_rate_limiting_str.lower() in (
                "true",
                "1",
                "yes",
            )
        ...
        if rate_limit_per_host_str:
            config_data["rate_limit_per_host"] = rate_limit_per_host_str.lower() in (
                "true",
                "1",
                "yes",
            )

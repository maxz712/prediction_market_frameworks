...
def main():
    ...
    print(
        f"✓ Signature validation: {'enabled' if config.enable_signature_validation else 'disabled'}"
    )
    ...
    print("\nHMAC Signature Example:")
    try:
        hmac_headers = signer.sign_request_hmac(method="GET", path="/markets", body="")
    ...
    print(
        f"✓ Current timestamp valid: {validator.validate_timestamp(current_timestamp)}"
    )
    ...
    print(
        f"✗ Invalid address format: {validator.validate_address_format(invalid_address)}"
    )
    ...
    auth_middleware = AuthMiddleware(
        validator=validator, max_timestamp_age=300, enable_nonce_tracking=True
    )
    ...
    print(
        f"✓ Nonce tracking: {'enabled' if config.nonce_tracking_enabled else 'disabled'}"
    )
    print(f"✓ Max signature age: {config.max_signature_age_seconds} seconds")
    print(
        f"✓ Request validation: {'enabled' if config.enable_signature_validation else 'disabled'}"
    )
